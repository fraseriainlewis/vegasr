...
result_logEv <- vegasBayesEvidence(
  ...
  errTol = 0.0001, maxIter = 10, seed = 99999, nsearch = 100000,
  extra_args=list(
    y=thedata$y,
    treat=thedata$treat,
...
```
```{r marg_beta111,include=TRUE,eval=TRUE}
...
p <- ggplot() +
  ...
  scale_color_manual(name = "Dataset", values = c("Original Log Ev" = "#8E44AD", "Marginal standardized" = "#2E86C1")) +
  scale_fill_manual(name = "Dataset", values = c("Original Log Ev" = "#D35400", "Marginal standardized" = "#2ECC71")) +
  
  # Labels and Title
  labs(
    title = "Marginal Posterior Density Estimation",
    subtitle = "Comparison of two integration models using Spline Interpolation",
    x = "Parameter Space (z)",
    y = "Density",
...
```{r}
plot(density(extract(stan_fit_2,"beta[1,1]")$"beta[1,1]"),main="beta[1,1]",col="blue",lwd=1)
...
#points(myz,as.matrix(ff_z),col="green",pch=21,bg="green")
#ff_interp <- splinefun(myz, as.matrix(ff_z), method = "fmm")
#lines(myz,ff_interp(myz),col="brown")
