...
result <- vegasBayesEvidence(
  ...
  nitn = 10, neval = 10000,
  errTol = 0.1, maxIter = 2, seed = 999199,fresh=TRUE,
  nsearch = 10000,
  extra_args=list(
    y=thedata$y,
    treat=thedata$treat,
...
result <- vegasBayesEvidence(
  f = vegasr::eigen_fn_log_post_2_par,
  lower = lower, upper = upper,
  nitn_warm = 1, neval_warm = 10000,
  nitn = 1, neval = 10000,
  errTol = 0.1, maxIter = 0, seed = 999199,fresh=FALSE,
  nsearch = 10000,
  extra_args=list(
    y=thedata$y,
    treat=thedata$treat,
...
inver<-function(x){
  #if(x>0){
  return( (sqrt(1+4*x^2)-1)/(2*x) )
  #} else {return( -(sqrt(1+4*x^2)-1)/(2*x) )}
  }

K <- length(unique(thedata$basket))
lower <- c(rep(-0.9999, 2*K), -0.9999, -0.9999, 1e-4, 1e-4)
upper <- c(rep( 0.9999, 2*K),  0.9999,  0.9999,   0.9999,   0.9999)
upper[6]<-inver(my25Stan["beta[1,2]"])
result25 <- vegasBayesEvidence(
  f = vegasr::eigen_fn_log_post_2_par,
  lower = lower, upper = upper,
...
cat("log evidence direct from vegas = ",result25$log_evidence,"\n")

cat("area under 2.5% from stan ", exp(result25$log_evidence-(result$log_evidence)),"\n")
