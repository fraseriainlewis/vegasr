...
result<-vegasBayesEvidence(f=vegasr:::eigen_fn_log_post_1_par,
...
f_x<-vegasr:::eigen_fn_log_post_1_par(myX,y=thedata$y,treat=thedata$treat,shiftby=result$shiftby,uselog=1.)
...
my_IP_sample<-sample.int(nrow(myX),size=500000,replace=TRUE,prob=wgts_std)
...
plot(density(myVar<-myX2[,i]/(1-myX2[,i]^2)),col="red")
i<-2
